var List = new Vue({
    ...
    methods: {
        ...
        /**
         * @method 切换分类
         * 根据点击的当前index去寻找goods当中等值的type，
         *即item.type === self.cate_index，分为一类
         ...
         */
        sortSales: function() {
            this.list.sort(this.compare('sales')); // sort参数为一个函数
        },
         ...
         */
        sortBy: function(method) { // 'setList' 'sortSales'  'sortPrice'

            this.filter_index = this.sortMethods.findIndex(function(item) {
                return item.method === method; // findIndex 返回符合条件的第一个元素的下标
            });
        ...
        addToCart: function(goods) {
            var alreadyIndex = Cart.cart.findIndex(function(item, index) {
                return item.id === goods.id; // goods.id点击添加进来的商品，item.id是遍历cart数组
                // 看看是否有与添加进来的商品相同，根据id判断。若存在，findIndex返回商品在cart数组中的位置
            });

            // 如果不存在则添加（不存在时，findIndex返回-1）
            if (alreadyIndex === -1) {
